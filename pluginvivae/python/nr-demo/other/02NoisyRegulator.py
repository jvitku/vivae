...
# Controller adds sin and cosin into the signal, then the noise is introduced by the ANN.
# This results into a pretty stochastic behaviour.
# 
# by Jaroslav Vitku
...
class Controller(simplemodule.SimpleModule):
    ...
    def termination_inputs(self,values):
        ...
        self.output[0] = self.output[0]+sin(self.t)
        self.output[1] = self.output[1]+cos(self.t)
...
net.connect(simulator.getAgent('a').getOrigin(),controller.getTermination('inputs'))    # connect agent sensors to the controller
A=net.make('ActuatorData',neurons=100,dimensions=2,radius=1.5)
net.connect(controller.getOrigin('outputs'), A)                                         # send control signal from controller into ANN 
net.connect(A,simulator.getAgent('a').getTermination())                                 # signal from ANN to agent
