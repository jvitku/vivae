 ...
 * @author Petr Smejkal
 * 
 * Slightly damaged by Jaroslav Vitku e.g. to be runnable in own stoppable thread;
 * arena can be ran synchronously with external agents communicating over the ROS network etc..  
 * (changes tagged by string ///my )
 * 
 */
...
public class Arena extends JPanel implements  KeyListener, Runnable{
	...
	private final boolean DEBUG_FRICTION_CACHE = false; ///my dunno what is it
	private String svgFileName = "";

	///my
	public void setInfiniteSimulation(){
		this.totalStepsPerSimulation = -1;
	}
	...
	public void loadScenario(String svgFileName) {
		...
		try {
			...
			loader.start();
			///		System.out.println("started, joining..");
			loader.join();
	...
	public void registerController(Active agent, VivaeController controller) {
		...
		///my register custom key listener (nice..)
		if(controller instanceof KeyControlledSensoryDataPublisher){
			if (parent != null) {
				parent.addKeyListener((KeyControlledSensoryDataPublisher) controller);
			}
			this.addKeyListener((KeyControlledSensoryDataPublisher) controller);
		}	
	}
	...
	/**
	 * THis is called when Thread.start() is called...implements Runnable..
	 * ///my
	 */
	@Override
	public void run(){
	 ...
	 * 
	 * Lets say that one step is one millisecond: 0.001 s
	 * 
	 */
	//@Override
	public void runArena() {
		///my
		collectROSAgents();
		if(isVisible)		//show arena at the beginning
			repaint();
		...
		while (isRunning && !shouldStop) {
			...
			this.waitForROSAgents();
			//System.out.println("done");
			moveVivaes();

			//System.out.println("Loopsleeptime: "+loopSleepTime);
			...
			///my: moved to the end
			///my: set totalStepsPerSimulation -1 in order to get infinite simulation
			if(totalStepsPerSimulation > 0)
	...
	ArrayList<RosAgent> agents;
	private final int maxWait = 100;	// max time to wait for message (ROS forgets some messages)
	
	/** 
	 * ///my
	 * Find all agents currently participating on the simulation and remember them.
	 */
	private void collectROSAgents(){
		this.agents = new ArrayList<RosAgent>();

		Vector<VivaeController> v = controllers;
		VivaeController a;

		for(int i=0; i<v.size(); i++){
			a = v.get(i);
			if(a instanceof RosAgent)
				agents.add((RosAgent)a);
		}
	}
	/**
	 * ///my
	 * Waits until all agents have received commands for velocity 
	 */
	private void waitForROSAgents(){
		RosAgent a;
		int poc;
		int sleeptime = 10;
		int maxAttempts = 100;
		...
		for(int i=0; i<agents.size(); i++){
			...
			while(!a.isReady()){
				if(poc++ > maxAttempts){
					if(this.shouldGiveUp(sleeptime,waited, a.getName()))
						break;
					try {
						Thread.sleep(sleeptime);
						if(waited%20==0 && waited>0)
							System.out.println("VIVAE: waiting for: "+waited+sleeptime+ " for agent "+a.getName());
					} catch (InterruptedException e) { e.printStackTrace(); }
				}
			}
		}
		// all agents ready, discard their state
		for(int i=0; i<agents.size(); i++){
			agents.get(i).discardReady();
		}
	}
	

	///my
	private boolean shouldGiveUp(int sleeptime,int waited, String name){
		if(!isRunning)
			return true;
		if(shouldStop)
			return true;
		if(sleeptime * waited > maxWait){
			System.out.println("Arena: enough.. skipping waiting for this agent: "+name);
			return true;
		}
		return false;
	}
