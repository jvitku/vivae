 ...
 *
 */
public class KeyControlledDataPublishingVivaeSim implements Simulation{
...
//	HashMap<String, Agent> agentMap;
	...
	public KeyControlledDataPublishingVivaeSim(){
	...
	public void setupAgentK(int number, /*double[][] wm,*/ double maxDistance, double frictionDistance) {
		...
		KeyControlledSensoryDataPublisher kc = new KeyControlledSensoryDataPublisher();
		arena.registerController(agent, kc);	// register agent (controller) in the arena

		if (agent instanceof FRNNControlledRobot) {
			System.out.println("registering the sensors..");
	...
	private void setupAgent(int number, double[][] wm, double maxDistance, double frictionDistance) {
		...
		double sangle = -Math.PI / 2;
		double ai = Math.PI / (snum / 2 - 1);
		System.out.println("au au: "+ai+" snum snum "+snum+" snangle: "+sangle+" neu "+neurons);
